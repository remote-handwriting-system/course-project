...
// ADC Configuration
#define FSR_ADC_CHANNEL ADC_CHANNEL_0  // GPIO 0 corresponds to ADC1 Channel 0 on ESP32-C6
...
esp_err_t force_reader_read_raw(int *raw_value) {
    ...
    esp_err_t ret = adc_oneshot_read(adc1_handle, FSR_ADC_CHANNEL, raw_value);
    if (ret != ESP_OK) {
